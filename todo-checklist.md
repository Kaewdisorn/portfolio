...
- [ ] Remove boilerplate: clear `app/page.tsx`, `app/globals.css` default styles, delete `public/vercel.svg`, `public/next.svg`

### 1.2 Install Dependencies

- [ ] `npm install next-mdx-remote gray-matter` — MDX content loading
- [ ] `npm install clsx` — conditional classNames utility
- [ ] Confirm `next`, `react`, `react-dom`, `typescript` versions are up to date

### 1.3 Configure TypeScript

- [ ] Set `"moduleResolution": "bundler"` in `tsconfig.json` (required for App Router)
- [ ] Confirm `"paths": { "@/*": ["./*"] }` is set
- [ ] Add `"noUncheckedIndexedAccess": true` for extra safety

### 1.4 Configure Tailwind

- [ ] Extend `tailwind.config.ts` with custom `fontFamily` for Inter
- [ ] Set `content` paths to include `app/**`, `components/**`, `content/**`
- [ ] Define custom colors: neutral base, accent, muted surface (2–3 colors max)
- [ ] Define custom `maxWidth` for prose reading width (e.g., `prose: '68ch'`)

### 1.5 Configure ESLint

- [ ] Confirm `eslint-config-next` is present
- [ ] Add `"no-restricted-syntax"` rule to ban `any` type
