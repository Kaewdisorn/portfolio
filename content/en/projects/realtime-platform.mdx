...
decisions:
  ...
  - title: "Protobuf serialization and Gzip compression for the GPS stream"
    body: "GPS frames arrive every second from every active device, creating a continuous high-frequency stream. JSON-encoded payloads at that frequency accumulate bandwidth cost and server-side parse overhead quickly. I replaced JSON with Protocol Buffers for GPS frame serialization and added Gzip compression on top. The combined change reduced network transfer volume by approximately 60% compared to the JSON baseline, with a measurable improvement in end-to-end response latency across the ingest pipeline."
challenges:
...
impact:
  ...
  - "Applied Protobuf serialization and Gzip compression to GPS frames, reducing network transfer volume by ~60% and improving end-to-end response latency"
---
...
### Protobuf serialization and Gzip for the ingest stream

GPS frames arrive every second from every active device. At that frequency, encoding overhead and payload size compound across the entire fleet. JSON is readable, but each GPS frame carries far more bytes than the data itself requires.

I replaced JSON with Protocol Buffers for GPS frame serialization and added Gzip compression on top. The combined effect reduced network transfer volume by approximately 60% compared to the JSON baseline. For a pipeline designed to stay as lightweight as possible at the ingest layer, reducing per-frame size also reduced pressure on the websocket handler, Redis, and downstream consumers throughout the processing chain.
...
- Applied Protobuf serialization and Gzip compression to GPS frames, cutting network transfer volume by ~60% and improving end-to-end response latency
