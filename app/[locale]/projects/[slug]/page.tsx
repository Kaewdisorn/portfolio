    ...
    <main id="main-content">
      <article className="py-10 sm:py-16">
        <div className="mx-auto w-full max-w-[var(--max-w-layout)] px-5 sm:px-8">
          ...
          {/* Header */}
          <header className="mb-10 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)] p-5 shadow-sm sm:p-8">
            <div className="mb-3 flex flex-wrap items-center gap-3 font-mono text-xs font-semibold text-[var(--color-text-muted)]">
            ...
            {project.stack.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {project.stack.map((tech) => (
                  <span
                    key={tech}
                    className="rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-1 font-sans text-xs font-semibold text-[var(--color-text)]"
                  >
                    {tech}
                  </span>
                ))}
            ...
            {project.repoUrl && (
              <div className="mt-5">
                <a
                  ...
                  className="inline-flex items-center gap-1.5 rounded-md border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-3 py-1.5 font-mono text-xs font-bold text-[var(--color-text)] transition-colors hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]"
                >
